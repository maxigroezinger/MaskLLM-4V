...
group.add_argument('--experiment', default='MaskLLM-4V', type=str, metavar='NAME',
...
group.add_argument('--sparsity-mode', type=str, default='dense', choices=['dense', 'sparse', 'maskllm'],
                   help='Sparsity mode')
...
def main():
    ...
    if args.sparse_checkpoint:
        print(f'Loading sparse checkpoint from {args.sparse_checkpoint}')
        checkpoint = torch.load(args.sparse_checkpoint, map_location='cpu')
        model.load_state_dict(checkpoint, strict=False if args.sparsity_mode == 'maskllm' else True)
